...
function stage1Complete(){
  ...
  s1Montage();
  goalClear();
  watch(S1_M_END-1200,'let it play.');
...
function s2ScoreOpened(){
  ...
  hideCue(); goalClear();
  watch(S2_OPEN_HOLD+2600,'let it play.');
...
function watch(ms,label){
  ...
  watchEnd();
  goalClear();
  el.querySelector('b').textContent=label||'let it play.';
...
function collapse(){
  ...
  s3BarClear();
  goalClear(); hideCue();
  watch(4600+INTERLUDE_MS+600,'let it play.');
  screenFlicker();
